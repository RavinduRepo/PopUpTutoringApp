...
class PlayerController(BaseController):
    ...
    def load_tutorial_from_dialog(self):
        ...
        file_path = filedialog.askopenfilename(
            title="Select Tutorial File", filetypes=[("JSON files", "*.json")]
        )

        if file_path:
            if self.load_tutorial_by_path(file_path):
                self.main_controller.views["play"].update_ui_on_load(
    ...
    def load_tutorial_by_path(self, file_path):
        ...
        if self.is_playing:
            messagebox.showwarning(
                ...
                "Please stop the current playback before loading a new tutorial.",
            )
            return False
        ...
        try:
            ...
            messagebox.showerror(
                "Load Error", f"Failed to load tutorial from '{file_path}': {e}"
            )
            self.current_tutorial = None
            return False
    ...
    def on_single_click(self, data):
        ...
        if self.is_paused or self.current_tutorial["steps"][self.current_step_index][
            "action_type"
        ].lower() not in ["left_click", "right_click"]:
            return
        x, y = data["x"], data["y"]
    ...
    def on_hotkey(self, data):
        ...
        if not self.is_paused and (
            ...
            or (action_type == "typing" and key_combo == "ctrl+v")
        ):
            self.main_controller.after(100, self.next_step)
    ...
    def _find_match_on_screen(self, template_image):
        ...
        with mss.mss() as sct:
            ...
            full_image_pil = Image.frombytes(
    ...
    def show_step(self):
        ...
        if (
            not self.is_playing
            or self.is_paused
            or self.current_step_index >= len(self.current_tutorial["steps"])
        ):
            self.end_playback()
            return
        ...
        if detected_coordinates:
            logger.info(
                f"Match found for step {self.current_step_index + 1} at {detected_coordinates}"
            )
            ...
            logger.info(
                f"No match found for step {self.current_step_index + 1}. Using recorded coordinates."
            )
        ...
        if action_type.lower() in ["left_click", "right_click", "double_click"]:
            ...
            if action_type.lower() == "typing":
                ...
                text_to_copy = self.current_tutorial["steps"][
